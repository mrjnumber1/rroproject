		...
		for( i = 0; i < MAX_GUILDCASTLE; i++ )
		{
			if( g->cas[i].changed )
			{
				if( SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `guild_rank` (`guild_id`, `castle_id`, "
					"`capture`, `emperium`, `treasure`, `top_eco`, `top_def`, `invest_eco`, `invest_def`, "
					"`possession_time`, `zeny_eco`, `zeny_def`, `skill_battleorder`, `skill_regeneration`, `skill_restore`, `skill_emergencycall`, "
					"`off_kill`, `off_death`, `def_kill`, `def_death`, `ext_kill`, `ext_death`) "
					"VALUES ('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d',  '%u', '%u', '%u', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
					g->guild_id, i,
					g->cas[i].capture, g->cas[i].emperium, g->cas[i].treasure, g->cas[i].top_eco, g->cas[i].top_def, g->cas[i].invest_eco, g->cas[i].invest_def, 
					g->cas[i].possession_time, g->cas[i].zeny_eco, g->cas[i].zeny_def, g->cas[i].skill_battleorder, g->cas[i].skill_regeneration, g->cas[i].skill_restore, g->cas[i].skill_emergencycall,
					g->cas[i].off.kill_count, g->cas[i].off.death_count, g->cas[i].def.kill_count, g->cas[i].def.death_count, g->cas[i].ext.kill_count, g->cas[i].ext.death_count) )
					Sql_ShowDebug(sql_handle);

				g->cas[i].changed = false;
			}
		}
	}
...
struct guild * inter_guild_fromsql(int guild_id)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle,
		"SELECT "
		"`castle_id`, `capture`, `emperium`, `treasure`, `top_eco`, `top_def`, `invest_eco`, `invest_def`, "
		"`possession_time`, `zeny_eco`, `zeny_def`, `skill_battleorder`, `skill_regeneration`, `skill_restore`, `skill_emergencycall`, "
		"`off_kill`, `off_death`, `def_kill`, `def_death`, `ext_kill`, `ext_death` "
	...
	else
	{
		while( SQL_SUCCESS == Sql_NextRow(sql_handle) )
		{
			...
			Sql_GetData(sql_handle, 1, &data, NULL); g->cas[i].capture = atoi(data);
			Sql_GetData(sql_handle, 2, &data, NULL); g->cas[i].emperium = atoi(data);
			Sql_GetData(sql_handle, 3, &data, NULL); g->cas[i].treasure = atoi(data);
			Sql_GetData(sql_handle, 4, &data, NULL); g->cas[i].top_eco = atoi(data);
			Sql_GetData(sql_handle, 5, &data, NULL); g->cas[i].top_def = atoi(data);
			Sql_GetData(sql_handle, 6, &data, NULL); g->cas[i].invest_eco = atoi(data);
			Sql_GetData(sql_handle, 7, &data, NULL); g->cas[i].invest_def = atoi(data);

			Sql_GetData(sql_handle, 8, &data, NULL); g->cas[i].possession_time = strtoul(data, NULL, 0);
			Sql_GetData(sql_handle, 9, &data, NULL); g->cas[i].zeny_eco = strtoul(data, NULL, 0);
			Sql_GetData(sql_handle,10, &data, NULL); g->cas[i].zeny_def = strtoul(data, NULL, 0);

			Sql_GetData(sql_handle,11, &data, NULL); g->cas[i].skill_battleorder = atoi(data);
			Sql_GetData(sql_handle,12, &data, NULL); g->cas[i].skill_regeneration = atoi(data);
			Sql_GetData(sql_handle,13, &data, NULL); g->cas[i].skill_restore = atoi(data);
			Sql_GetData(sql_handle,14, &data, NULL); g->cas[i].skill_emergencycall = atoi(data);

			Sql_GetData(sql_handle,15, &data, NULL); g->cas[i].off.kill_count = atoi(data);
			Sql_GetData(sql_handle,16, &data, NULL); g->cas[i].off.death_count = atoi(data);
			Sql_GetData(sql_handle,17, &data, NULL); g->cas[i].def.kill_count = atoi(data);
			Sql_GetData(sql_handle,18, &data, NULL); g->cas[i].def.death_count = atoi(data);
			Sql_GetData(sql_handle,19, &data, NULL); g->cas[i].ext.kill_count = atoi(data);
			Sql_GetData(sql_handle,20, &data, NULL); g->cas[i].ext.death_count = atoi(data);
...
int guild_calcinfo(struct guild *g)
{
	...
	g->max_member = DEFAULT_GUILD_MEMBERS + guild_checkskill(g, GD_EXTENSION) * MEMBERS_PER_GUILD_EXTENSION; 
	if(g->max_member > MAX_GUILD)
	{	
...
int mapif_parse_CreateGuild(int fd,int account_id,char *name,struct guild_member *master)
{
	...
	strcpy(g->position[0].name,"Guild Master");
...
int mapif_parse_Guild_Save_Score(int fd, int guild_id, int index, struct guild_rank_data *grd)
{
	...
	if( (g = inter_guild_fromsql(guild_id)) == NULL )
		return 0;
	memcpy(&g->cas[index], grd, sizeof(struct guild_rank_data));
	g->save_flag |= GS_RANKING;
