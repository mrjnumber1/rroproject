...
int inter_recv_packet_length[] = {
	-1,-1, 7,-1, -1,17,36, 0,  0, 0, 0, 0,  0, 0,  0, 0,	// 3000-
...
int mapif_account_reg_reply(int fd,int account_id,int char_id, int type)
{
	...
	if(reg->reg_num==0){
		...
		int i,p;
		for (p=17,i = 0; i < reg->reg_num && p < 5000; i++) {
...
int mapif_parse_Registry(int fd)
{
	...
	inter_accreg_tosql(RFIFOL(fd,4),RFIFOL(fd,8),reg, RFIFOB(fd,16));
...
{
	//Load Char Registry
	if (RFIFOB(fd,16)) mapif_account_reg_reply(fd,RFIFOL(fd,2),RFIFOL(fd,6),3);
	//Load Account Registry
	if (RFIFOB(fd,15)) mapif_account_reg_reply(fd,RFIFOL(fd,2),RFIFOL(fd,6),2);
	//Ask Login Server for Account2 values.
	if (RFIFOB(fd,14)) request_accreg2(RFIFOL(fd,2), RFIFOL(fd, 6), RFIFOL(fd,10));
	return 1;
}
