...
int status_damage(struct block_list *src,struct block_list *target,int hp, int sp, int walkdelay, int flag, int skillid)
{
	struct status_data *status;
...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	switch (target->type) {
		case BL_PC:  flag = pc_dead((TBL_PC*)target,src,skillid); break;
	...
	if( sc && sc->data[SC_KAIZEL] && (!map_flag_gvg(target->m) || !(map[target->m].flag.battleground)))
...
int status_heal(struct block_list *bl,int hp,int sp, int flag)
{
	...
	case BL_PC:  pc_heal((TBL_PC*)bl,hp,sp,0); break;
	...
	if (!(flag&8)) {
		...
		if(sd && SC_COMMON_MIN <= type && type <= SC_COMMON_MAX)
		{
			...
			if( sd->reseff[type-SC_COMMON_MIN] > 0 )
				bonus_resist += sd->reseff[type-SC_COMMON_MIN];
			if( sd->sc.data[SC_COMMONSC_RESIST] )
				bonus_resist += sd->sc.data[SC_COMMONSC_RESIST]->val1*100;
				
			if (bonus_resist)
				rate -= rate*bonus_resist/10000;
		}
	}
	if (!(rand()%10000 < rate))
		return 0;
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_BGWINTAUNT:
		if( sd )
		{
			...
			{
				//clif_emotion(bl,E_NO1);
				clif_specialeffect(&sd->bl, 508, AREA); // 508 = fireworks
				sc_timer_next(200+tick, status_change_timer, bl->id,data);
			}
			else
				status_change_end(bl,SC_BGWINTAUNT,INVALID_TIMER);
		}
	...
	case SC_BGLOSER:
		if( sd )
		{
			if(sd->state.bg_loser)
			{
				...
				status_change_end(bl, SC_BGLOSER, INVALID_TIMER);
		}
		break;
...
static int status_natural_heal(struct block_list* bl, va_list args)
{
	...
	if (flag&RGN_HP)
	{
		...
		if(regen->tick.hp >= (unsigned int)battle_config.natural_healhp_interval)
		{
			...
			do {
				val += regen->hp;
				val += val/25; // on here we subtly give players 125% regen power :)
