...
void map_reqnickdb(struct map_session_data * sd, int charid)
{
	...
	if(BG_CHARID && BG_CHARID == charid)
	{
		clif_solved_charname(sd->fd, charid, BG_CHARNAME);
		return;
	}
	if(WOE_CHARID && WOE_CHARID == charid)
	{
		clif_solved_charname(sd->fd, charid, WOE_CHARNAME);
		return;
	}
	if(RAID_CHARID && RAID_CHARID == charid)
	{
		clif_solved_charname(sd->fd, charid, RAID_CHARNAME);
		return;
	}
...
int map_getcellp(struct map_data* m,int x,int y,cell_chk cellchk)
{
	...
	if(x<=1 || x>=m->xs-2 || y<=1 || y>=m->ys-2) // Mark the outer 2 columns/rows of maps as unwalkable. [Valaris for KarmaRO]
