...
int battle_delay_damage_sub(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	if ( target && dat && target->prev != NULL && !status_isdead(target) ) {
		...
		{
			map_freeblock_lock();
			status_fix_damage(dat->src, target, dat->damage, dat->delay, dat->skill_id);
...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv)
{
	...
	if( sc && sc->count )
	{
		...
		if((sce=sc->data[SC_REJECTSWORD]) && flag&BF_WEAPON &&
			...
			status_fix_damage(bl,src,damage,clif_damage(bl,src,gettick(),0,0,damage,0,0,0), skill_num);
...
void battle_consume_ammo(TBL_PC*sd, int skill, int lv)
{
	...
	if(sd->equip_index[EQI_AMMO]>=0) //Qty check should have been done in skill_check_condition
	{
		...
		else if( sd->state.bg_id && count_bg_stats(sd->bl.m) )
			add2limit(sd->status.bg_round_stats.ammo_used, qty, UINT_MAX);
	}
...
// eAthena additions
	...
	{ "cashshop_show_points",               &battle_config.cashshop_show_points,            1,      0,      1,              },
	...
	{ "bg_idle_announce",                   &battle_config.bg_idle_announce,                60,     0,      INT_MAX,        },
	...
	{ "bg_ranked_max_games",                &battle_config.bg_ranked_max_games,             20,     10,     100,            },
	
	{ "raid_update_interval",               &battle_config.raid_update_interval,            1000,   100,    INT_MAX,        },
	{ "raid_short_attack_damage_rate",      &battle_config.raid_short_damage_rate,          80,     0,      INT_MAX,        },
	{ "raid_long_attack_damage_rate",       &battle_config.raid_long_damage_rate,           80,     0,      INT_MAX,        },
	{ "raid_weapon_attack_damage_rate",     &battle_config.raid_weapon_damage_rate,         60,     0,      INT_MAX,        },
	{ "raid_magic_attack_damage_rate",      &battle_config.raid_magic_damage_rate,          60,     0,      INT_MAX,        },
