...
int chrif_save(struct map_session_data *sd, int flag)
{
	...
	if (sd->state.storage_flag == 3)
		storage_member_storagesave(sd->status.account_id, sd->status.member_id, flag);
...
int chrif_updatefamelist(struct map_session_data* sd, short flag)
{
	...
	else type = 3 + flag; // 4 = BG Ranked | 5 = BG Normal
	...
	switch( flag )
	{
	case 1:  WFIFOL(char_fd,6) = sd->status.bg_stats.rank_points; break;
	case 2:  WFIFOL(char_fd,6) = sd->status.bg_stats.points; break;
...
int chrif_recvfamelist_single(int fd, int type)
{
	...
	switch( type )
	{
		...
		case 4: memset(bgrank_fame_list, 0, sizeof(bgrank_fame_list)); list = bgrank_fame_list;  break;
		case 5: memset(bg_fame_list, 0, sizeof(bg_fame_list)); list = bg_fame_list;  break;
		default: return 0;
	}
...
int chrif_updatefamelist_ack(int fd)
{
	...
	switch (RFIFOB(fd,2))
	{
		...
		case 4: list = bgrank_fame_list;  break;
		case 5: list = bg_fame_list;  break;
		default: return 0;
	}
	index = RFIFOB(fd, 3);
