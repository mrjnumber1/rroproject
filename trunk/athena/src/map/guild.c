...
int guild_send_xy_timer_sub(DBKey key,void *data,va_list ap)
{
	...
	for(i=0;i<g->max_member;i++){
		...
		if( sd != NULL && sd->fd && (sd->guild_x != sd->bl.x || sd->guild_y != sd->bl.y) && !sd->state.bg_id )
		{
			clif_guild_xy(sd);
			sd->guild_x = sd->bl.x;
			sd->guild_y = sd->bl.y;
...
// Blocks all guild skills which have a common delay time.
int guild_block_skill_end(int tid, unsigned int tick, int id, intptr data)
{
	struct guild *g;
	char output[128];
...
int guild_gm_changed(int guild_id, int account_id, int char_id)
{
	...
	for( i = 0; i < g->max_member; i++ )
	{
		if( g->member[i].sd && g->member[i].sd->fd && !g->member[i].sd->state.bg_id)
		{
...
int guild_ranking_save(void)
{
	...
	{ // Castle Score Update
		...
		if( (g = guild_search(gc->guild_id)) != NULL )
		{
			...
			g->cas[index].possession_time += addtime;
			g->cas[index].defensive_score += score;

			g->cas[index].changed = true;
		}
	}
	...
	for( g = (struct guild*)iter->first(iter,NULL); iter->exists(iter); g = (struct guild*)iter->next(iter,NULL) )
	{
		for( i = 0; i < MAX_GUILDCASTLE; i++ )
		{
			...
			intif_guild_save_score(g->guild_id, i, &g->cas[i]);
