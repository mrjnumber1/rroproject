...
int mail_removeitem(struct map_session_data *sd, short flag)
{
	...
	if( sd->mail.amount )
	{
		if (flag)
		{ // Item send
			log_pick_pc(sd, LOG_TYPE_MAIL, sd->mail.nameid, -sd->mail.amount, &sd->status.inventory[sd->mail.index], sd->status.inventory[sd->mail.index].serial);
...
void mail_getattachment(struct map_session_data* sd, int zeny, struct item* item)
{
	if( item->nameid > 0 && item->amount > 0 )
	{
		...
		log_pick_pc(sd, LOG_TYPE_MAIL, item->nameid, item->amount, item, item->serial);
...
void mail_deliveryfail(struct map_session_data *sd, struct mail_message *msg)
{
	...
	if( msg->item.amount > 0 )
	{
		...
		log_pick_pc(sd, LOG_TYPE_MAIL, msg->item.nameid, msg->item.amount, &msg->item, msg->item.serial);
