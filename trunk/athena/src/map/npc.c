...
int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points)
{
	...
	if( !nd || (nd->subtype != CASHSHOP && nd->subtype != ITEMSHOP && nd->subtype != VARSHOP) )
		return 1;
	...
	if(nd->subtype == ITEMSHOP)
	{
		if( points > 0)
		{
			ShowWarning("Player %s (%d:%d) sent a hexed packet trying to buy points from an Item Shop.", sd->status.name, sd->status.account_id, sd->status.char_id);
			return 6;
		}
	...
	else if(nd->subtype == VARSHOP)
	{
		if(points > 0)
		{
			ShowWarning("Player %s (%d:%d) sent a hexed packet trying to buy points from a Variable Shop.", sd->status.name, sd->status.account_id, sd->status.char_id);
			return 6;
		}

		if( (pc_readregistry(sd,nd->cash_var,nd->cash_vartype)  < price))
			return 6;

		pc_setregistry(sd, nd->cash_var, pc_readregistry(sd, nd->cash_var,nd->cash_vartype) - price, nd->cash_vartype);
	}
	else if(nd->subtype == CASHSHOP)
	{ 
		int mypoints = pc_readaccountreg2(sd, "##KAFRAPOINTS");
		int mycash = pc_readaccountreg2(sd, "##CASHPOINTS");
		if( (mypoints < points) || (mycash < price - points) )
			return 6;
...
int npc_cashshop_buylist(struct map_session_data *sd, int points, int count, unsigned short* item_list)
{
	...
	else if(nd->subtype == VARSHOP)
	{
		...
		if(nd->cash_var)
		{
			if(pc_readregistry(sd, nd->cash_var, nd->cash_vartype) < vt)
				return 6;
			
			pc_setregistry(sd,nd->cash_var,pc_readregistry(sd,nd->cash_var,nd->cash_vartype) - vt,nd->cash_vartype);
		}
	}
	else if(nd->subtype == CASHSHOP)
	{
		int mypoints = pc_readaccountreg2(sd, "##KAFRAPOINTS");
		int mycash = pc_readaccountreg2(sd, "##CASHPOINTS");
		if( (mypoints < points) || (mycash < vt - points) )
			return 6;
...
int npc_buylist(struct map_session_data* sd, int n, unsigned short* item_list)
{
	...
	for( i = 0; i < n; ++i )
	{
		...
		log_pick_pc(sd, LOG_TYPE_NPC, item_tmp.nameid, amount, NULL, item_tmp.serial);
		//Logs
	}

	// custom merchant shop exp bonus
	if( battle_config.shop_exp > 0 && z > 0 && ( (skill = pc_checkskill(sd,MC_DISCOUNT)) > 0 || (skill = pc_checkskill(sd,RG_COMPULSION)) > 0 )  )
	{
...
int npc_selllist(struct map_session_data* sd, int n, unsigned short* item_list)
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		log_pick_pc(sd, LOG_TYPE_NPC, nameid, -amount, &sd->status.inventory[idx], sd->status.inventory[idx].serial);
		//Logs
...
int npc_unload(struct npc_data* nd)
{
	...
	if( (nd->subtype == SHOP || nd->subtype == CASHSHOP || nd->subtype == ITEMSHOP || nd->subtype == VARSHOP) && nd->src_id == 0) //src check for duplicate shops [Orcao]
	{
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	{// w1=<map name>,<x>,<y>,<facing>
		...
		case ITEMSHOP:
			...
			||	strchr(w4, ',') == NULL )
			{
				ShowError("npc_parse_shop: Invalid item shop definition in file '%s', line '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n", filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
				return strchr(start,'\n');// skip and continue
			}
			break;		
		case VARSHOP:
	...
	if( type == ITEMSHOP && cashitem >= 0 && itemdb_exists(cashitem) == NULL)
	{
	...
	if(type == VARSHOP)
	{
		char cashvarname_tmp[32];

		if (sscanf(w2,"varshop(%32[^)])",cashvarname_tmp) == 1)
		{
			nd->cash_var = aStrdup(cashvarname_tmp);
					
			if(nd->cash_var[0] == '#' && nd->cash_var[1] == '#')
				nd->cash_vartype = 1;
			else if (nd->cash_var[0] == '#')
				nd->cash_vartype = 2;
			else
				nd->cash_vartype = 3;
		}
...
int npc_convertlabel_db(DBKey key, void* data, va_list ap)
{
	...
	int lpos = (int)__64BPTRSIZE(data);
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	// get placement
	if( (type==SHOP || type==CASHSHOP || type==ITEMSHOP || type==VARSHOP || type==SCRIPT) && strcmp(w1, "-") == 0 )
	...
	case VARSHOP:
		nd->cash_var = dnd->cash_var;
		nd->cash_vartype = dnd->cash_vartype;
	case ITEMSHOP:
	case SHOP:
	case CASHSHOP:
		++npc_shop;
		nd->cashitem = dnd->cashitem;
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	else if (!strcmpi(w3,"allow_raid_items"))
		map[m].flag.allow_raid_items=state;
	else if (!strcmpi(w3,"allow_gvg_items"))
...
void npc_parsesrcfile(const char* filepath)
{
	...
	for( p = skip_space(buffer); p && *p ; p = skip_space(p) )
	{
		...
		else if( (!strcasecmp(w2,"shop") || !strcasecmp(w2,"cashshop") || !strcasecmp(w2,"itemshop") || (i=0, sscanf(w2,"varshop%n",&i), (i > 0 && w2[i] == '(')) ) && count > 3 )
		{
