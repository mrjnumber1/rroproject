...
BUILDIN_FUNC(countitem)
{
	...
	for(i = 0; i < MAX_INVENTORY; i++)
	{
		...
		if(sd->status.inventory[i].nameid == nameid)
		{
			...
			added_count = sd->status.inventory[i].amount;
		}
		
		if(added_count && sd->status.inventory[i].card[0] == CARD0_CREATE )
		{
			int char_id = MakeDWord(sd->status.inventory[i].card[2],sd->status.inventory[i].card[3]);
...
BUILDIN_FUNC(getitem)
{
	...
	for (i = 0; i < amount; i += get_count)
	{
		...
		log_pick_pc(sd, LOG_TYPE_SCRIPT, nameid, amount, NULL, it.serial);
		it.serial = 0;
     }
...
BUILDIN_FUNC(getitem2)
{
	...
	if(nameid > 0) {
		...
		//Logs items, got from (N)PC scripts [Lupus]
		log_pick_pc(sd, LOG_TYPE_SCRIPT, nameid, amount, &item_tmp, item_tmp.serial);
...
BUILDIN_FUNC(getnameditem)
{
	...
	//Logs items, got from (N)PC scripts [Lupus]
	log_pick_pc(sd, LOG_TYPE_SCRIPT, item_tmp.nameid, item_tmp.amount, &item_tmp, item_tmp.serial);
...
static void buildin_delitem_delete(struct map_session_data* sd, int idx, int* amount, bool delete_items)
{
	...
	if( delete_items )
	{
		...
		//Logs items, got from (N)PC scripts [Lupus]
		log_pick_pc(sd, LOG_TYPE_SCRIPT, inv->nameid, -delamount, inv, inv->serial);
		//Logs
...
BUILDIN_FUNC(memberopenstorage)
{
	TBL_PC* sd;
	int ret;
	...
	ret = storage_member_storageopen(sd);
	script_pushint(st, ret);
	return 0;
}
/*==========================================
		...
		switch(index){
			case 1:
				...
				{ // Current WoE
					...
					g->cas[i].possession_time += addtime;
					g->cas[i].defensive_score += score;
					g->cas[i].changed = true;
				}
			...
			case 2:
				{
					...
					if( g && gc->economy < value )
					{
						...
						add2limit(g->cas[gc->castle_id].invest_eco, eco, USHRT_MAX);
						if( g->cas[gc->castle_id].top_eco < value )
							g->cas[gc->castle_id].top_eco = value;
			...
			case 3:
				{
					...
					if( g && gc->defense < value )
					{
						...
						add2limit(g->cas[gc->castle_id].invest_def, def, USHRT_MAX);
						if( g->cas[gc->castle_id].top_def < value )
							g->cas[gc->castle_id].top_def = value;
						g->cas[gc->castle_id].changed = true;
						if( !agit_flag )
						{
							intif_guild_save_score(g->guild_id, gc->castle_id, &g->cas[gc->castle_id]);
...
BUILDIN_FUNC(guild_addzenyeco)
{
	...
	add2limit(g->cas[gc->castle_id].zeny_eco, value, UINT_MAX);
	g->cas[gc->castle_id].changed = true;
	if( !agit_flag )
	{
		intif_guild_save_score(g->guild_id, gc->castle_id, &g->cas[gc->castle_id]);
...
BUILDIN_FUNC(guild_addzenydef)
{
	...
	add2limit(g->cas[gc->castle_id].zeny_def, value, UINT_MAX);
	g->cas[gc->castle_id].changed = true;
	if( !agit_flag )
	{
		intif_guild_save_score(g->guild_id, gc->castle_id, &g->cas[gc->castle_id]);
...
BUILDIN_FUNC(successremovecards)
{
	...
	for( c = sd->inventory_data[i]->slot - 1; c >= 0; --c )
	{
		...
		{// extract this card from the item
			...
			//Logs items, got from (N)PC scripts [Lupus]
			log_pick_pc(sd, LOG_TYPE_SCRIPT, item_tmp.nameid, 1, NULL, item_tmp.serial);
	...
	{	// カードを取り除いたアイテム所得
		...
		//Logs items, got from (N)PC scripts [Lupus]
		log_pick_pc(sd, LOG_TYPE_SCRIPT, item_tmp.nameid, 1, &item_tmp, item_tmp.serial);
...
BUILDIN_FUNC(clearitem)
{
	...
	for (i=0; i<MAX_INVENTORY; i++) {
		if (sd->status.inventory[i].amount) {
			...
			//Logs items, got from (N)PC scripts [Lupus]
			log_pick_pc(sd, LOG_TYPE_SCRIPT, sd->status.inventory[i].nameid, -sd->status.inventory[i].amount, &sd->status.inventory[i], sd->status.inventory[i].serial);
...
BUILDIN_FUNC(getnpcid)
{
	...
	if(!npc_id)
	{
		ShowError("buildin_npcid: no npc oid attached.");
		script_pushint(st, -1);
	}
	...
	{ 
	case 0:								
		script_pushint(st, npc_id);
		break;
	case 1:
		script_pushint(st, nd->u.scr.guild_id);
		break;
	case 2:
		script_pushint(st, nd->u.scr.bg_id);
		break;
	case 3:
		script_pushint(st, nd->chat_id);
                break;
	default:
		ShowError("buildin_npcid: unknown type requested %d", type);
		script_pushint(st, -1);
		return -1;
		break;
	}
...
BUILDIN_FUNC(npcshopitem)
{
	...
	if( !nd || ( nd->subtype != SHOP && nd->subtype != CASHSHOP && nd->subtype != VARSHOP) )
	{	//Not found.
		script_pushint(st,0);
		return 0;
	}
...
BUILDIN_FUNC(bg_check_best)
{
	struct battleground_data *bg1, *bg2;
	int bg_id1, bg_id2;

	bg_id1 = script_getnum(st,2);
	bg_id2 = script_getnum(st,3);
	...
	if( (bg2 = bg_team_search(bg_id2)) == NULL )
	{
...
BUILDIN_FUNC(bg_classcount)
{
	...
	if( ((bg = bg_team_search(bg_id)) == NULL) || pc_jobid2mapid(jobid) < 0 )
	{
		ShowError("buildin_bg_classcount: Invalid Team %d or jobid %d requested.", bg_id, jobid);
		script_pushint(st, -1);
		return 0;
	}
	...
	for(i=0; i < MAX_BG_MEMBERS; i++ )
	{
		...
		if( (bg_sd = bg->members[i].sd) == NULL )
			continue;

		if(bg_sd->status.class_ == jobid)
			++result;
	}
...
BUILDIN_FUNC(sc_check) // sc_check sc_type, (valnum?)
{
	int id, type;
	struct map_session_data* sd = script_rid2sd(st);
...
BUILDIN_FUNC(homun_heal)
{
	...
	status_heal(&sd->hd->bl, hp, sp, 0);

	return 0;
}

BUILDIN_FUNC(homun_percentheal)
{
	int hp,sp;
	TBL_PC* sd;
	...
	status_percent_heal(&sd->hd->bl, hp, sp);
...
BUILDIN_FUNC(adjust_memberreg)
{
	...
	delta = script_getnum(st,2);
	reg = script_getstr(st,3);

	
	script_pushint(st, pc_adjust_memreg(sd, reg, delta));
	return 0;
}
BUILDIN_FUNC(givecash)
{
	TBL_PC* sd = script_rid2sd(st);
	int amt;
...
BUILDIN_FUNC(paycash)
{
	...
	if(!amt)
	{
		ShowError("buildin_paycash: negative/invalid amt %d requested from player %d\n", amt, sd->status.char_id);
		script_pushint(st, -1);
		return 0;
	}
...
BUILDIN_FUNC(paypoints)
{
	...
	if(!amt)
	{
		ShowError("buildin_paypoints: negative/invalid amt %d requested from player %d\n", amt, sd->status.char_id);
		script_pushint(st, -1);
	}

	pc_paycash(sd, amt, amt);
