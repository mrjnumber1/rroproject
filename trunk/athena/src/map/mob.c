...
int mobdb_searchname(const char *str)
{
	...
	for(i=0;i<=MAX_MOB_DB;i++){
		mob = mob_db(i);
		if( mob == mob_dummy || mob_is_clone(i) ) //Skip dummy mobs.
			continue;
...
static bool mob_ai_sub_hard(struct mob_data *md, unsigned int tick)
{
	...
	{	//Loot time.
		...
		log_pick_mob(md, LOG_TYPE_LOOT, fitem->item_data.nameid, fitem->item_data.amount, &fitem->item_data, fitem->item_data.serial);
...
static struct item_drop* mob_setdropitem(int nameid, int qty)
{
	...
	drop->item_data.identify = 1;//itemdb_isidentified(nameid);
	drop->next = NULL;
	return drop;
};
...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, unsigned short flag)
{
	...
	if (loot)
		log_pick_mob(md, LOG_TYPE_LOOT, ditem->item_data.nameid, -ditem->item_data.amount, &ditem->item_data, ditem->item_data.serial);
	else
		log_pick_mob(md, LOG_TYPE_PICKDROP_MONSTER, ditem->item_data.nameid, -ditem->item_data.amount, NULL, ditem->item_data.serial);
	...
	if( sd 
		&& (drop_rate <= sd->state.autoloot || mob_process_aloot(sd, ditem->item_data.nameid))
		&& (mob_process_noloot(sd, ditem->item_data.nameid))
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if(mvp_sd && md->db->mexp > 0 && !md->special_state.ai)
	{
		...
		else
		{
			...
			for(j=0;j<3;j++)
			{
				...
				item.identify= 1; //itemdb_isidentified(item.nameid);
				...
				log_pick_mob(md, LOG_TYPE_PICKDROP_MONSTER, item.nameid, -1, NULL, item.serial);

				if (!temp)
					log_pick_pc(mvp_sd, LOG_TYPE_PICKDROP_PLAYER, item.nameid, 1, NULL, item.serial);
				break;
			}
			log_mvpdrop(mvp_sd, md->class_, log_mvp);

			if(mvp_sd->status.party_id)
			{
			...
			mvp_sd->mvp_kills += 1;
			pc_setglobalreg(mvp_sd, "MVP_Kills", mvp_sd->mvp_kills);
			// kafra points on mvp kill
			// ex: randgris will give 285 kafra points to the mvper on 10x mvpexp bonus rates.
			amt = md->db->mexp / 50000;
			pc_getcash(mvp_sd, 0, amt);
	...
	else // give the client some time to breathe and display shit with corpse eg soul drain
...
int mob_clone_spawn(struct map_session_data *sd, int m, int x, int y, const char *event, int master_id, int mode, int flag, unsigned int duration)
{
	...
	//temporarily set fd of sd to 0 so it doesn't receive mssages from skill_check_condition_castbegin
	fd = sd->fd;
	sd->fd = 0;
	...
	for (i=0,j = MAX_SKILL_TREE-1;j>=0 && i< MAX_MOBSKILL ;j--) {
		...
		// set it so that the clone can only use skills the original player could
...
static bool mob_parse_dbrow(char** str)
{
	...
	// To give extra bonus EXP to MVPs
	if(db->mexp)
	{
		exp = (double)db->base_exp * (double)battle_config.base_exp_rate / 100.;
		db->base_exp = (unsigned int)cap_value(exp, 0, UINT_MAX);
	
		exp = (double)db->job_exp * (double)battle_config.job_exp_rate / 100.;
		db->job_exp = (unsigned int)cap_value(exp, 0, UINT_MAX);
	}
