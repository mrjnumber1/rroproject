...
struct map_session_data {
	...
	struct {
		...
		unsigned int rest : 1;
		unsigned int storage_flag : 2; //0: closed, 1: Normal Storage open, 2: guild storage open 3: mem storage open[Skotlex]
		...
		unsigned short autolootid[MAX_ALOOTITEM]; // [Zephyrus]
		unsigned short nolootid[MAX_ALOOTITEM];
		unsigned int autolootactive : 1;
		unsigned int nolootactive : 1;
	...
	// these variables should NOT exist as part of sd
	// to prevent exploits via member reg updates
	...
	struct 
	{
		...
		unsigned short deaths, kills;
	} expinfo;
	unsigned short mvp_kills;
