...
static bool mmo_auth_fromsql(AccountDB_SQL* db, struct mmo_account* acc, int account_id)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle,
	    "SELECT `account_id`,`userid`,`user_pass`,`sex`,`email`,`level`,`state`,`unban_time`,`expiration_time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`member_id` FROM `%s` WHERE `account_id` = %d",
	...
	Sql_GetData(sql_handle, 13, &data, NULL); acc->member_id = atoi(data);
	Sql_FreeResult(sql_handle);


	// retrieve account regs for the specified user
	// we use account_reg2s instead as member regs.
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `str`,`value` FROM `%s` WHERE `type`='1' AND `account_id`='%d'", db->accreg_db, acc->member_id) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
static bool mmo_auth_tosql(AccountDB_SQL* db, const struct mmo_account* acc, bool is_new)
{
	...
	if( SQL_SUCCESS != Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `type`='1' AND `account_id`='%d' ", db->accreg_db, acc->member_id) )
	{
		Sql_ShowDebug(sql_handle);
		break;
	}
	// insert new account regs
	if( SQL_SUCCESS != SqlStmt_Prepare(stmt, "INSERT INTO `%s` (`type`, `account_id`, `str`, `value`) VALUES ( 1 , '%d' , ? , ? );",  db->accreg_db, acc->member_id) )
	{
		SqlStmt_ShowDebug(stmt);
		break;
	}
