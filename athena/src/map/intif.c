...
int intif_saveregistry(struct map_session_data *sd, int type)
{
	...
	WFIFOHEAD(inter_fd, (256+32) * MAX_REG_NUM+17);
	WFIFOW(inter_fd,0)=0x3004;
	WFIFOL(inter_fd,4) = sd->status.account_id;
	WFIFOL(inter_fd,8) = sd->status.char_id;
	WFIFOL(inter_fd,12)= sd->status.member_id;
	WFIFOB(inter_fd,16)=type;
	for( p = 17, i = 0; i < count; i++ ) {
...
int intif_request_registry(struct map_session_data *sd, int flag)
{
	...
	WFIFOL(inter_fd,2) = sd->status.account_id;
	WFIFOL(inter_fd,6) = sd->status.char_id;
	WFIFOL(inter_fd,10) = sd->status.member_id;
	WFIFOB(inter_fd,14) = (flag&1?1:0); //Request Acc Reg 2
	WFIFOB(inter_fd,15) = (flag&2?1:0); //Request Acc Reg
	WFIFOB(inter_fd,16) = (flag&4?1:0); //Request Char Reg
...
int intif_send_member_storage(int account_id, struct member_storage_data *mstor)
{
	...
	WFIFOHEAD(inter_fd,sizeof(struct member_storage_data)+12);
	WFIFOW(inter_fd,0) = 0x3017;
	WFIFOW(inter_fd,2) = (unsigned short)sizeof(struct member_storage_data)+12;
	WFIFOL(inter_fd,4) = account_id;
	WFIFOL(inter_fd,8) = mstor->member_id;
	memcpy( WFIFOP(inter_fd,12), mstor, sizeof(struct member_storage_data) );
	WFIFOSET(inter_fd,WFIFOW(inter_fd,2));
...
int intif_parse_Registers(int fd)
{
	...
	struct global_reg *reg;
	int *qty;
	int account_id = RFIFOL(fd,4), char_id = RFIFOL(fd,8), member_id = RFIFOL(fd,12);
	...
	if (node)
		...
		if (sd && RFIFOB(fd,16) == 3 && sd->status.char_id != char_id)
...
int intif_parse_LoadMemberStorage(int fd)
{
	...
	if(sd == NULL)
	{
		ShowError("intif_parse_LoadMemberStorage: user not found %d\n",RFIFOL(fd,4));
		return 1;
	}

	mstor = member2storage(member_id);

	if(!mstor) 
	{
		ShowWarning("intif_parse_LoadMemberStorage: error member_id %d does no exist\n", member_id);
		return 1;
	}

	if(mstor->storage_status == 1)
	{
		ShowWarning("intif_parse_LoadMemberStorage: storage received for a client already open! (User %d:%d)\n", sd->status.account_id, sd->status.char_id);
		return 1;
	}
	if(mstor->dirty)
	{
		ShowWarning("intif_parse_LoadMemberStorage: received storage for an already modified non-saved storage! (User %d:%d)\n", sd->status.account_id, sd->status.char_id);
		return 1;
	}
	if(RFIFOW(fd,2)-12 != sizeof(struct member_storage_data) )
	{
		ShowError("intif_parse_LoadMemberStorage: data size error: %d %d\n",RFIFOW(fd,2)-12, sizeof(struct member_storage_data));
		mstor->storage_status = 0;
		return 1;
	}

	memcpy(mstor, RFIFOP(fd,12), sizeof(struct member_storage_data));
	storage_member_storageopen(sd);
	return 0;
}
...
int intif_parse_SaveMemberStorage(int fd)
{
	storage_member_storagesaved(RFIFOL(fd,6));
	return 0;
}
...
int intif_parse(int fd)
{
	...
	case 0x3816:	intif_parse_LoadMemberStorage(fd); break;
	case 0x3817:	intif_parse_SaveMemberStorage(fd); break;
