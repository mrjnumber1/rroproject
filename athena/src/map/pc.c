...
/*
#define MAX_HP_POTS 5
#define MAX_SP_POTS 6
#define MAX_STATUS_POTS 3
#define MAX_ASPD_POTS 7
#define MAX_SPEED_POTS 1
#define MAX_FOODS (6*10+6*2+3+3)
#define MAX_BOXES 8
#define MAX_CONVERTERS 5
#define MAX_RESIST_POTS 5
#define MAX_SCROLLS 16

#define CTZC_ARROWID(id) ( ((id) >= 1750 && (id) <= 1774) || ((id) >= 13200 && (id) <= 13259) )
#define CTZC_ACIDID 7136
#define CTZC_GRENADEID  7135
#define CTZC_YGEMID ITEMID_YELLOW_GEMSTONE
#define CTZC_RGEMID ITEMID_RED_GEMSTONE
#define CTZC_BGEMID ITEMID_BLUE_GEMSTONE
#define CTZC_TRAPID ITEMID_TRAP
#define CTZC_EDPID 678

const int hp_pots[MAX_HP_POTS] = { 504, 522, 526, 547, 11501};
const int sp_pots[MAX_SP_POTS] = { 505, 514, 526, 533, 578, 11502};
const int status_pots[MAX_STATUS_POTS] =  { 506, 525, 526};
const int aspd_pots[MAX_ASPD_POTS] = { 645, 656, 657, 678, 14509, 14510, 14511}; 
const int speed_pots[MAX_SPEED_POTS] = { 12016 };

const int foods[MAX_FOODS] = { 
	...
	12319, 12320, 12321};
const int boxes[MAX_BOXES] = { 12027, 12028, 12029, 12030, 12031, 12032, 12033, 12034};
const int converters[MAX_CONVERTERS] = { 12114, 12115, 12116, 12117, 12020};
const int resist_pots[MAX_RESIST_POTS] = { 12118, 12119, 12120, 12121, 12279 };
const int scrolls[MAX_SCROLLS] = { 12215, 12216, 12217, 12218, 12219, 12220, 14512, 14513, 14514, 14515, 14516, 14517, 14518, 14519, 14520, 14593 };
*/
...
void pc_addfame(struct map_session_data *sd,int count, short flag)
{
	...
	switch( flag )
	{
	case 1: // Bg Ranked Matches
		{
			int old = (sd->status.bg_stats.rank_points)/100;


			add2limit(sd->status.bg_stats.rank_points,count,MAX_FAME);

			if(old != (sd->status.bg_stats.rank_points)/100)
			{
				clif_rank_info(sd,( (sd->status.bg_stats.rank_points)/100) - old, sd->status.bg_stats.rank_points, flag);
			}
	...
	case 2: // Bg Normal Matches
		{
			...
			add2limit(sd->status.bg_stats.points,count,MAX_FAME);

			if(old != (sd->status.bg_stats.points)/100)
			{
				clif_rank_info(sd, ( (sd->status.bg_stats.points)/100)- old , sd->status.bg_stats.points, flag);
			}
...
void pc_setmemberid(struct map_session_data *sd)
{
	...
	StringBuf buf;
	StringBuf_Init(&buf);
	StringBuf_Printf(&buf, "SELECT `login`.`member_id` FROM `login` WHERE `login`.`account_id`='%d' LIMIT 1",sd->status.account_id);
	if(SQL_ERROR == Sql_Query(mmysql_handle, StringBuf_Value(&buf)) )
	{
		Sql_ShowDebug(mmysql_handle);
		ShowWarning("Error finding Member id for AID %d CID %d.\n", sd->status.account_id, sd->status.char_id);
		clif_displaymessage(sd->fd, "You need to contact a GM immediately.");
	}
	else
...
//	sd->kafraPoints = pc_readaccountreg2(sd,"##KAFRAPOINTS");

	// Cooking Exp
	sd->cook_mastery = pc_readglobalreg(sd,"COOK_MASTERY");
	sd->mvp_kills = pc_readglobalreg(sd, "MVP_Kills");
...
void pc_fix_items(struct map_session_data *sd)
{
	...
	safestrncpy(msg.body, "An item you possessed was modified. If you feel this was an error please contact a GM.", MAIL_BODY_LENGTH);
	msg.status = MAIL_NEW;
	msg.zeny = 0;
...
{
	char output[128];
	int cash, cur_cashpoints, cur_kafrapoints;
	nullpo_retv(sd);
	...
	if( cur_cashpoints < cash || cur_kafrapoints < points )
	{
		ShowError("pc_paycash: Not enough points (cash=%d, kafra=%d) to cover the price (cash=%d, kafra=%d) (account_id=%d, char_id=%d).\n", cur_cashpoints, cur_kafrapoints, cash, points, sd->status.account_id, sd->status.char_id);
		return;
	}

	pc_setaccountreg2(sd, "##CASHPOINTS", cur_cashpoints-cash);
	pc_setaccountreg2(sd, "##KAFRAPOINTS", cur_kafrapoints-points);

	if( battle_config.cashshop_show_points )
	{
		sprintf(output, msg_txt(504), points, cash, pc_readaccountreg2(sd, "##KAFRAPOINTS"), pc_readaccountreg2(sd, "##CASHPOINTS"));
		clif_disp_onlyself(sd, output, strlen(output));
	}
}

int pc_adjust_memreg(struct map_session_data *sd, const char* reg, int delta)
{
	int old;

	nullpo_retr(-1, sd);

	old = pc_readaccountreg2(sd, reg);

	if(delta < 0)
	{
		if( (old - delta) < 0)
		{
			ShowError("pc_adjust_memreg: cannot have negative mreg! reg:%s old:%d delta:%d, aid:%d cid:%d mid:%d", reg, old, delta, sd->status.account_id, sd->status.char_id, sd->status.member_id);
			return -1;
		}
	}
	else if(delta > 0)
	{
		if( (old + delta > INT_MAX) )
		{
			ShowError("pc_adjust_memreg: mreg too large! reg:%s old:%d delta:%d aid:%d cid:%d mid:%d", reg, old, delta, sd->status.account_id, sd->status.char_id, sd->status.member_id);
			return -1;
		}
	}

	
	return (pc_setaccountreg2(sd, reg, old + delta));
}

void pc_getcash(struct map_session_data *sd, int cash, int points)
{
	char output[128];
	int cur_cashpoints, cur_kafrapoints;
	nullpo_retv(sd);
	...
	if( cash > 0 )
	{
		if( cash > MAX_ZENY-cur_cashpoints )
		{
			ShowWarning("pc_getcash: Cash point overflow (cash=%d, have cash=%d, account_id=%d, char_id=%d).\n", cash, cur_cashpoints, sd->status.account_id, sd->status.char_id);
			cash = MAX_ZENY-cur_cashpoints;
		}
		...
		if( battle_config.cashshop_show_points )
		{
			sprintf(output, msg_txt(505), cash, cur_cashpoints+cash);
			clif_disp_onlyself(sd, output, strlen(output));
		}
	}
	else if( cash < 0 )
	...
	if( points > 0 )
	{
		if( points > MAX_ZENY- cur_kafrapoints )
		{
		...
		if( battle_config.cashshop_show_points )
		{
			sprintf(output, msg_txt(506), points, cur_kafrapoints+points);
			clif_disp_onlyself(sd, output, strlen(output));
		}
	}
	else if( points < 0 )
...
int pc_search_inventory(struct map_session_data *sd,int item_id)
{
	...
	if( item_id )
	{
		...
		else if( map_raid_items(sd->bl.m) && RAID_CHARID )
		{
			ARR_FIND( 0, MAX_INVENTORY, i, sd->status.inventory[i].nameid == item_id && sd->status.inventory[i].amount > 0 && sd->status.inventory[i].card[0] == CARD0_CREATE && MakeDWord(sd->status.inventory[i].card[2],sd->status.inventory[i].card[3]) == RAID_CHARID );
			if( i < MAX_INVENTORY ) return i;
		}
	}
	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		...
		{
			if( !map_bg_items(sd->bl.m) && BG_CHARID && BG_CHARID == char_id )
				continue;
			if( !map_gvg_items(sd->bl.m) && WOE_CHARID && WOE_CHARID == char_id )
				continue;
			if( !map_raid_items(sd->bl.m) && RAID_CHARID && RAID_CHARID == char_id )
				continue;
		}
...
int pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	log_pick_pc(sd, LOG_TYPE_PICKDROP_PLAYER, sd->status.inventory[n].nameid, -amount, (struct item*)&sd->status.inventory[n], sd->status.inventory[n].serial);
	//Logs
	...
	if ( sd->state.dropinfo ) 
	{
			...
			char output[250];
			if( ( idata = itemdb_exists( sd->status.inventory[ n ].nameid ) ) != NULL ) {
			sprintf(output, "[- You dropped %s [%d] (%d)-]", idata->jname, idata->slot, amount);
			clif_displaymessage (sd->fd, output); }
	}
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	if( sd->status.inventory[n].card[0] == CARD0_CREATE 
		&& char_id 
		&& (char_id == BG_CHARID || char_id == WOE_CHARID || char_id == RAID_CHARID || pc_famerank(char_id, MAPID_ALCHEMIST) ) )
	{
...
int pc_cart_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	{	// BG/WoE/Raid Items
		...
		if( (BG_CHARID && char_id == BG_CHARID && BG_TRADE&16) || (WOE_CHARID && char_id == WOE_CHARID && WOE_TRADE&16) || (RAID_CHARID && char_id == RAID_CHARID && RAID_TRADE&16) )
		{
			clif_displaymessage (sd->fd, msg_txt(264));
			return 1;
		}
...
int pc_steal_item(struct map_session_data *sd,struct block_list *bl, int lv)
{
	...
	tmp_item.amount = 1;
	tmp_item.identify = 1; //itemdb_isidentified(itemid);
	...
	log_pick_mob(md, LOG_TYPE_PICKDROP_MONSTER, itemid, -1, NULL, tmp_item.serial);
	log_pick_pc(sd, LOG_TYPE_PICKDROP_PLAYER, itemid, 1, NULL, tmp_item.serial);
...
int pc_gainexp(struct map_session_data *sd, struct block_list *src, unsigned int base_exp,unsigned int job_exp,bool quest)
{
	...
	if(sd->state.showexp) 
	{
		...
		int start_time, day =0, hour=0, minute = 0, second = 0;

		sprintf(output,
			"EXP Gained - Base:%u (%.2f%%) | Job:%u (%.2f%%)",base_exp,nextbp*(float)100,job_exp,nextjp*(float)100);
		clif_disp_onlyself(sd,output,strlen(output));

		if( (start_time = DIFF_TICK(last_tick,sd->expinfo.start_time)) <= 0)
			return 1;
...
void pc_record_mobkills(struct map_session_data *sd, struct mob_data *md)
{
	...
	if( count_bg_stats(sd->bl.m) && sd->state.bg_id )
	{
		struct battleground_data *bg;
		int i;
		if( (bg = bg_team_search(sd->state.bg_id)) == NULL )
			return;
		ARR_FIND(0,MAX_BG_MEMBERS,i,bg->members[i].sd == sd);
		if( i >= MAX_BG_MEMBERS )
			return;
		type = bg->members[i].ranked ? 1 : 2;
	}
	...
	if( map_allowed_woe(sd->bl.m) )
	{
		...
		{
		case MOBID_EMPERIUM:
			add2limit(sd->status.woe_stats.emperium_kill, 1, USHRT_MAX);
			if( (g = guild_search(sd->status.guild_id)) && (gc = guild_mapindex2gc(map[sd->bl.m].index)) )
			{
				add2limit(g->cas[gc->castle_id].emperium, 1, USHRT_MAX);
				g->cas[gc->castle_id].changed = true;
			}
			break;
		case MOBID_BARRICADE1:
			add2limit(sd->status.woe_stats.barricade_kill, 1, USHRT_MAX);
			break;
		case MOBID_GUARDIAN_STONE1:
		case MOBID_GUARDIAN_STONE2:
			add2limit(sd->status.woe_stats.gstone_kill, 1, USHRT_MAX);
			break;
		case 1285:
		case 1286:
		case 1287:
		case 1899:
		case 1900:
			add2limit(sd->status.woe_stats.guardian_kill, 1, USHRT_MAX);
			break;
		}
		return;
	}
	else if( count_bg_stats(sd->bl.m) && sd->state.bg_id)
	{
		switch( md->class_ )
		{
		case MOBID_BG_GUARDIAN_STONE1:
		case MOBID_BG_GUARDIAN_STONE2:
			add2limit(sd->status.bg_round_stats.gstone_kill, 1, USHRT_MAX);
			pc_addfame(sd,500,type);
			break;
		case MOBID_BG_EMPERIUM:
			add2limit(sd->status.bg_round_stats.emperium_kill, 1, USHRT_MAX);
			pc_addfame(sd,1000,type);
			break;
		case MOBID_BG_BARRICADE:
			add2limit(sd->status.bg_round_stats.barricade_kill, 1, USHRT_MAX);
			pc_addfame(sd,100,type);
			break;
		}
		return;
	}
	...
	if( map[sd->bl.m].flag.gvg_castle && ((md->class_ >= 1324 && md->class_ <= 1363) || (md->class_ >= 1938 && md->class_ <= 1946)) && (g = guild_search(sd->status.guild_id)) && (gc = guild_mapindex2gc(map[sd->bl.m].index)) )
	{
		...
		if( !agit_flag )
		{
			intif_guild_save_score(g->guild_id, gc->castle_id, &g->cas[gc->castle_id]);
			g->cas[gc->castle_id].changed = false;
		}
		return;
	}
...
void pc_record_maxdamage(struct block_list *src, struct block_list *dst, int damage)
{
	...
	{
		if( count_bg_stats(src->m) && sd->state.bg_id && sd->status.bg_round_stats.top_damage < damage )
		{
			sd->status.bg_round_stats.top_damage = damage;

			if(sd->status.bg_stats.top_damage < damage)
			{
				int old = sd->status.bg_stats.top_damage;
		...
		else if( map_allowed_woe(src->m) && sd->status.woe_stats.top_damage < damage )
		{
			int old = sd->status.bg_stats.top_damage;
...
void pc_record_damage(struct block_list *src, struct block_list *dst, int damage)
{
	...
	if(map[src->m].flag.battleground && sd->state.bg_id)
	{
		struct battleground_data *bg;
		int i;
		if( (bg = bg_team_search(sd->state.bg_id)) == NULL )
		...
		case BL_MOB:
		{
			...
			if( map_allowed_woe(src->m) && md->guardian_data )
			{
				switch( md->class_ )
				{
					case MOBID_EMPERIUM:
					case MOBID_BG_EMPERIUM:
						add2limit(sd->status.woe_stats.emperium_damage, damage, UINT_MAX);
						break;
					case MOBID_BARRICADE1:
					case MOBID_BG_BARRICADE:
						add2limit(sd->status.woe_stats.barricade_damage, damage, UINT_MAX);
						break;
					case MOBID_GUARDIAN_STONE1:
					case MOBID_GUARDIAN_STONE2:
					case MOBID_BG_GUARDIAN_STONE1:
					case MOBID_BG_GUARDIAN_STONE2:
						add2limit(sd->status.woe_stats.gstone_damage, damage, UINT_MAX);
						break;
					default:
						add2limit(sd->status.woe_stats.guardian_damage, damage, UINT_MAX);
						break;
				}
			}
			else if(count_bg_stats(src->m) && sd->state.bg_id)
			{
				switch( md->class_)
				{
					case MOBID_BG_GUARDIAN_STONE1:
					case MOBID_BG_GUARDIAN_STONE2:
						add2limit(sd->status.bg_round_stats.gstone_damage, damage, UINT_MAX);
						break;
					case MOBID_BG_EMPERIUM:
						add2limit(sd->status.bg_round_stats.emperium_damage, damage, UINT_MAX);
						break;
					case MOBID_BG_BARRICADE:
						add2limit(sd->status.bg_round_stats.barricade_damage, damage, UINT_MAX);
						break;
				}
			}
			break;
...
void pc_update_kill_ranking(struct map_session_data *tsd, struct map_session_data *ssd, int skill)
{
	int m;
	int type;
	...
	if( map_allowed_woe(m) )
	{
		...
		add2limit(ssd->status.woe_stats.kill_count, 1, USHRT_MAX);
		add2limit(tsd->status.woe_stats.death_count, 1, USHRT_MAX);
		...
		{ // Offensive Ranking - Killing Castle Owners
			add2limit(sg->cas[i].off.kill_count, 1, UINT_MAX);
			add2limit(tg->cas[i].def.death_count, 1, UINT_MAX);
		}
		else if( ssd->status.guild_id == gc->guild_id )
		{ // Defensive Ranking - Killing Castle Invaders
			add2limit(sg->cas[i].def.kill_count, 1, UINT_MAX);
			add2limit(tg->cas[i].off.death_count, 1, UINT_MAX);
		}
		else
		{ // Killing other guilds invaders
			add2limit(sg->cas[i].ext.kill_count, 1, UINT_MAX);
			add2limit(tg->cas[i].ext.death_count, 1, UINT_MAX);
		}

		tg->cas[i].changed = true;
		sg->cas[i].changed = true;

	}
	else if( ssd->state.bg_id && tsd->state.bg_id  && count_bg_stats(m))
	{
		/*==========================================
		...
		if( (s_bg = bg_team_search(ssd->state.bg_id)) == NULL || (t_bg = bg_team_search(tsd->state.bg_id)) == NULL )
			return;

		log_bg_kill(ssd,tsd,skill);


		ARR_FIND(0,MAX_BG_MEMBERS,i,s_bg->members[i].sd == ssd);
		if( i >= MAX_BG_MEMBERS )
			return;
		type = s_bg->members[i].ranked ? 1 : 2;


		add2limit(ssd->status.bg_round_stats.kill_count, 1, USHRT_MAX);
		add2limit(tsd->status.bg_round_stats.death_count, 1, USHRT_MAX);
		...
		case MAPID_CREATOR:
			...
			case MAPID_PROFESSOR:
				points = 25;
				if(tsd->class_ == MAPID_CLOWNGYPSY && tsd->status.sex == SEX_FEMALE)
					points = 15;
				break;
			default:
				points = 10;
...
//	if( sd->state.bg_id && sd->state.bg_afk && (bg = bg_team_search(sd->state.bg_id)) != NULL && bg->g )
...
int pc_dead(struct map_session_data *sd,struct block_list *src, int skillid)
{
	int i=0,j=0,k=0;
	unsigned int tick = gettick();
	short flag = 0;
	...
	{
		struct battleground_data *bg;
		if( count_bg_stats(sd->bl.m) && (bg = bg_team_search(sd->state.bg_id)) != NULL && bg->die_event[0] )
		{
		//	pc_setreg(sd,add_str("@killer_bg_id"),bg_team_get_id(src)); // Killer's Team
		//	pc_setreg(sd,add_str("@killer_bg_src"),ssd && ssd->state.bg_id ? ssd->bl.id : 0);
	...
	if (src && src->type == BL_PC)
	{
		...
		pc_update_kill_ranking(sd, ssd, skillid); // Ranking System
...
int pc_itemheal(struct map_session_data *sd,int itemid, int hp,int sp)
{
	...
	if(hp) {
		...
		if( sd->status.guild_id && map_allowed_woe(sd->bl.m) )
			add2limit(sd->status.woe_stats.hp_heal_potions, 1, UINT_MAX);
		else if( count_bg_stats(sd->bl.m) && sd->state.bg_id )
			add2limit(sd->status.bg_round_stats.hp_heal_potions, 1, UINT_MAX);
	}
	if(sp) {
		...
		if( sd->status.guild_id && map_allowed_woe(sd->bl.m) )
			add2limit(sd->status.woe_stats.sp_heal_potions, 1, UINT_MAX);
		else if( count_bg_stats(sd->bl.m) && sd->state.bg_id )
			add2limit(sd->status.bg_round_stats.sp_heal_potions, 1, UINT_MAX);
	}
...
int pc_jobchange(struct map_session_data *sd,int job, int upper)
{
	...
	if (b_class&JOBL_2) {
		if (!(sd->class_&JOBL_2))
			...
			sd->change_level = 50; //Assume 50 for us
...
int pc_candrop(struct map_session_data *sd,struct item *item)
{
	...
	{
		int char_id = MakeDWord(item->card[2],item->card[3]);
		if(    (BG_CHARID && char_id == BG_CHARID && BG_TRADE&1) 
			|| (WOE_CHARID && char_id == WOE_CHARID && WOE_TRADE&1) 
			|| (RAID_CHARID && char_id == RAID_CHARID && RAID_TRADE&1) 
		  )
			return 0;
	}
...
int pc_readmemreg(struct map_session_data *sd, const char *reg)
{
#ifndef TXT_ONLY
	int result = 0;
	StringBuf buf;
	...
	else
	{
		...
		{
			char* data;
			Sql_GetData(mmysql_handle, 0, &data, NULL); result = atoi(data);
		}
	}
...
int pc_setregistry(struct map_session_data *sd,const char *reg,int val,int type)
{
	...
	break;
	case 1: //Account2 reg
		if( !strcmp(reg,"##CASHPOINTS") && pc_readaccountreg2(sd, "##CASHPOINTS") != val )
			val = cap_value(val, 0, MAX_ZENY);

		else if( !strcmp(reg,"##KAFRAPOINTS") && pc_readaccountreg2(sd, "##KAFRAPOINTS") != val )
			val = cap_value(val, 0, MAX_ZENY);

		pc_setmemreg(sd,reg, val);
		return 1;
		//sd_reg = sd->save_reg.account2;
		//max = &sd->save_reg.account2_num;
...
int pc_setregistry_str(struct map_session_data *sd,const char *reg,const char *val,int type)
{
	...
	case 1: //Account2 reg
		...
		ShowError("pc_setregistry_str : attempted to set str memreg.\n");
		return 0;
	break;
	default:
		return 0;
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos)
{
	...
	if( sd->sc.data[SC_BERSERK] || sd->sc.data[SC_BGLOSER])
	{
		clif_equipitemack(sd,n,0,0);	// fail
		return 0;
	}
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	if( !(flag&2) && sd->sc.count && (sd->sc.data[SC_BERSERK] || sd->sc.data[SC_BGLOSER]) )
	{
...
int pc_init_ctz(struct map_session_data *sd)
{
	...
	for(i=0; i < BGT_MAX; ++i)
	{
		sd->bg_stats.wins[i] = 0;
		sd->bg_stats.losses[i] = 0;
		sd->bg_stats.ties[i] = 0;
		sd->bg_stats.quits[i] = 0;
	}
	...
	for(i=0; i < CTZ_MAX; ++i)
	{
		...
		for(j=0; j < MAX_SKILL; ++j)
			sd->status.skill[j].ctz_casts[i] = 0;
		
		sd->ctz_deaths[i] = 0;

		for(j=0; j < CTZU_MAX; ++j)
		{
			sd->ctz_damages[i][j].kills = 0;
			sd->ctz_damages[i][j].top_done = 0;
			sd->ctz_damages[i][j].top_received = 0;
			sd->ctz_damages[i][j].total_done = 0;
			sd->ctz_damages[i][j].total_received = 0;
		}

		sd->ctz_consumables[i].acids = 0;
		sd->ctz_consumables[i].grenades = 0;
		sd->ctz_consumables[i].arrows = 0;
		sd->ctz_consumables[i].traps = 0;
		sd->ctz_consumables[i].edps = 0;
		sd->ctz_consumables[i].ygems = 0;
		sd->ctz_consumables[i].bgems = 0;
		sd->ctz_consumables[i].rgems = 0;
		for(j=0; j < CTZCT_MAX; ++j)
			sd->ctz_consumables[i].usables[j] = 0;
		sd->ctz_consumables[i].zeny = 0;
	}
...
{
	enum e_consumable_trace_zone zone;

	if(sd->bg_id && map[sd->bl.m].flag.battleground)
		zone = CTZ_BG;
	else if(map_flag_gvg((sd->bl.m)))
		zone = CTZ_WOE;
	else if(map[sd->bl.m].instance_id)
		zone = CTZ_INSTANCE;
	else
		zone = CTZ_NONE;
...
int pc_ctz_countcast(struct map_session_data *sd, enum e_consumable_trace_zone zone, int skillid)
{
	char msg[200];

	snprintf(msg, 200, "Player used skill %d in zone %d", skillid, zone);
	clif_displaymessage(sd->fd, msg);
	if(zone == CTZ_NONE)
	{
		ShowError("Incorrect pc_ctz_countcast(sd, %d, %d)", zone, skillid);
		return 0;
	}

	sd->status.skillcount[zone][

	return 1;
}

int pc_ctz_increment(struct map_session_data *sd, enum e_consumable_trace_zone zone, int nameid, int quantity)
{
	char msg[200];
	int i = 0;
	int type;
	...
	sprintf(msg, "pc_ctz_increment used %d (%d ea) in zone %d", nameid, quantity, zone);
	clif_displaymessage(sd->fd, msg);

	if(zone == CTZ_NONE) //just in case
		return 0;
	


	if		(nameid == CTZC_ACIDID) //arrows/bullets + venom knives
	{
		sd->ctz_consumables[zone].acids += quantity;
	}
	else if (nameid == CTZC_GRENADEID)
	{
		sd->ctz_consumables[zone].grenades += quantity;
	}
	else if (nameid == CTZC_YGEMID)
	{
		sd->ctz_consumables[zone].ygems += quantity;
	}
	else if (nameid == CTZC_RGEMID)
	{
		sd->ctz_consumables[zone].rgems += quantity;
	}
	else if (nameid == CTZC_BGEMID)
	{
		sd->ctz_consumables[zone].bgems += quantity;
	}
	else if (nameid == CTZC_TRAPID)
	{
		sd->ctz_consumables[zone].traps += quantity;
	}
	else if (nameid == CTZC_EDPID)
	{
		sd->ctz_consumables[zone].edps += quantity;
	}
	else if (nameid == CTZC_GRENADEID)
	{
		sd->ctz_consumables[zone].acids += quantity;
	}
	else if (CTZC_ARROWID(nameid))
	{
		sd->ctz_consumables[zone].arrows += quantity;
	}
	else //non ammo item
	{
		...
		while(type < CTZCT_MAX  && found == false) 
		{
			int max_index = 0;
			switch(type)
			{
				case CTZCT_HP:
					p = hp_pots; max_index = MAX_HP_POTS; break;
				case CTZCT_SP:
					p = sp_pots; max_index = MAX_SP_POTS; break;
				case CTZCT_ASPD:
					p = aspd_pots; max_index = MAX_ASPD_POTS; break;
				case CTZCT_STATUS:
					p = status_pots; max_index = MAX_STATUS_POTS; break;
				case CTZCT_SPEED:
					p = speed_pots; max_index = MAX_SPEED_POTS; break;
				case CTZCT_FOOD:
					p = foods; max_index = MAX_FOODS; break;
				case CTZCT_BOX:
					p = boxes; max_index = MAX_BOXES; break;
				case CTZCT_CONVERTER:
					p = converters; max_index = MAX_CONVERTERS; break;
				case CTZCT_RESIST:
					p = resist_pots; max_index = MAX_RESIST_POTS; break;
				case CTZCT_SCROLL:
					p = scrolls; max_index = MAX_SCROLLS; break;
			}

			for(i = 0; i < max_index && type != CTZCT_NONE; ++i)
			{
				if(nameid == p[i])
				{
					char msg[100];

					if(nameid == CTZC_EDPID && sd->class_ != MAPID_ASSASSIN_CROSS) // EDP bottle shouldn't count as ASPD pot for non sinx
						continue; 

					snprintf(msg, 100, "Declared player used item %d", nameid);
					clif_displaymessage(sd->fd, msg);
					found = true;
					break;
				}
			...
			{	
				char msg[100];
				snprintf(msg, 100, "Updating used item %d (%d ea)", nameid, quantity);
				clif_displaymessage(sd->fd, msg);
				
				sd->ctz_consumables[zone].usables[i] += quantity;
			}
...
int pc_getbgwins(struct map_session_data *sd){
	...
	for(i=0; i < BGT_MAX; ++i)
	{
		num += sd->bg_stats.wins[i];
	}

	return num;
}

int pc_getbglosses(struct map_session_data *sd){

	int num=0;
	int i;

	for(i=0; i < BGT_MAX; ++i)
		num += sd->bg_stats.losses[i];

	return num;
}
int pc_getbgties(struct map_session_data *sd){

	int num=0;
	int i;

	for(i=0; i < BGT_MAX; ++i)
		num += sd->bg_stats.ties[i];
...
void pc_ranking_reset(int type, bool char_server)
{
	...
	for( sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		case 0: // WoE Ranking Reset
			memset(&sd->status.woe_stats, 0, sizeof(struct s_woestats));
			memset(&sd->status.woe_skillcount, 0, sizeof(struct s_skillcount));
			break;
		case 1: // Battleground Stats
			memset(&sd->status.bg_stats, 0, sizeof(struct s_battleground_stats));
