...
void buyingstore_trade(struct map_session_data* sd, int account_id, unsigned int buyer_id, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		if( sd->status.inventory[index].card[0] == CARD0_CREATE 
			&& (char_id = MakeDWord(sd->status.inventory[index].card[2],sd->status.inventory[index].card[3])) > 0 
			&& ((char_id == BG_CHARID && BG_TRADE&2) || (char_id == WOE_CHARID && WOE_TRADE&2) || (char_id == RAID_CHARID && RAID_TRADE&2) ) 
			)
		{ // Items where creator's ID is important
			clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED, nameid);
			clif_displaymessage(sd->fd,"Cannot trade event reserved items (BG/WoE).");
			return;
		}
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		// log
		log_pick_pc(sd, LOG_TYPE_BUYING_STORE, nameid, -((int)amount), &sd->status.inventory[index], sd->status.inventory[index].serial);
		log_pick_pc(pl_sd, LOG_TYPE_BUYING_STORE, nameid, amount, &sd->status.inventory[index], sd->status.inventory[index].serial);
