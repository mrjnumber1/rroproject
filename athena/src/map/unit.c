			...
			{// replaced redundant foreachinrange call with smaller and much more efficient iteration
				for( i = 0; g->max_member > i; i++ )
				{
					...
					{// perform the aura on the member as appropriate
						skill_guildaura_sub( g->member[i].sd, g->member[i].sd->bl.id, strvit, agidex );
					}
				}
			}
		}
...
int unit_attack(struct block_list *src,int target_id,int continuous)
{
	...
	if( src->type == BL_PC )
	{
		...
		if( pc_is90overweight(sd) || sd->sc.data[SC_BGLOSER] )
...
// Do not reset can-act delay. [Skotlex]
	...
	case BL_PC:
	{
		...
		if(sd->state.storage_flag == 1)
			...
			storage_member_storage_quit(sd,0);
	...
	if( bl->type != BL_MOB || !status_isdead(bl)) // mob clearing handled with mob_dead unless the monster isn't dead
		clif_clearunit_area(bl,clrtype);
	map_delblock(bl);
	map_freeblock_unlock();
	return 1;
