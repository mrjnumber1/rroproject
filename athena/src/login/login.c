...
int parse_fromchar(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			int member_id = RFIFOL(fd,8);
			ShowStatus("We got to 0x2728 bitch\n");

			if( !accounts->load_num(accounts, &acc, account_id) )
				ShowStatus("Char-server '%s': receiving (from the char-server) of account_reg2 (account: %d mem: %d not found, ip: %s).\n", server[id].name, account_id, member_id, ip);
			else
			{
				int len;
				int p;
				ShowNotice("char-server '%s': receiving (from the char-server) of account_reg2 (account: %d, mem: %d ip: %s).\n", server[id].name, account_id, member_id, ip);
				for( j = 0, p = 17; j < ACCOUNT_REG2_NUM && p < RFIFOW(fd,2); ++j )
				{
				...
				return 0;
		{
			...
			int member_id = RFIFOL(fd,10);
			//ShowInfo("we got here!\n");
			RFIFOSKIP(fd,14);
			...
			WFIFOL(fd,8) = char_id;
			WFIFOL(fd,12) = member_id;
			WFIFOB(fd,16) = 1; //Type 1 for Account2 registry
...
int do_init(int argc, char** argv)
{
	...
	if( client_hash_active == false ) 
		ShowWarning("Client hash check is disabled! Consider enabling it or FACE MY WRATH.\n");
