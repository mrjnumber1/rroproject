...
enum e_ctz_units
{
	CTZU_ENEMIES, //hom/merc damages go uncounted by design
	CTZU_STONE,
	CTZU_BARRICADE,
	CTZU_EMPERIUM,
...
struct s_ctz_damage {
	unsigned int kills;
	unsigned int top_done, top_received;
	unsigned int total_done, total_received;
};
struct s_ctz_consumables {
	unsigned int acids, grenades, arrows, traps, edps;
	unsigned int ygems, bgems, rgems;
	unsigned int usables[CTZCT_MAX];
	//unsigned int hp_pots, sp_pots, aspd_pots, status_pots, speed_pots;
	//unsigned int foods, boxes, converters, resist_pots, scrolls;
	unsigned long zeny;
};

struct s_ctz_skillcount {
...
*/
	...
	unsigned short
		...
		deserter;
	int points, rank_points, rank_games;
};
...
//	struct member_storage_data mem_storage;
	...
	struct s_battleground_stats bg_stats, bg_round_stats;
	struct s_skillcount bg_skillcount[MAX_SKILL_TREE]; 
...
struct guild {
	...
	struct guild_rank_data cas[MAX_GUILDCASTLE];
